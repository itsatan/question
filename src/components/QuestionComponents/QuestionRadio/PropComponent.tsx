...
const RadioPropComponent: React.FC<QuestionRadioPropsType> = (props: QuestionRadioPropsType) => {
	...
	const handleValuesChange = () => {
		...
		if (newValues.options) {
			...
			newValues.options = newValues.options.filter(opt => !(opt.text === undefined))
		}
											...
											rules={[
												{ required: true, message: '请输入选项文字' },
												{
													validator: (_, text) => {
														...
														let num = 0
														options.forEach((opt: OptionsType) => {
